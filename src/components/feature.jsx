...
export default function Feature() {
  ...
  const { center, radius, isSmallScreen } = useMemo(() => {
    ...
    return { isSmallScreen: small, center: small ? 300 : 560, radius: small ? 220 : 500 };
  }, []);
  ...
  const positions = useMemo(() => {
    return images.map((_, i) => {
      ...
      const x = center + radius * Math.cos(theta) - (isSmallScreen ? 70 : 120);
      const y = center + radius * Math.sin(theta) - (isSmallScreen ? 90 : 120);
      return { x, y };
    });
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[100vw] sm:max-w-[1200px] h-[400px] sm:h-[600px] flex items-center justify-center z-40">
          {images.map((img, i) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 w-full h-20 sm:h-40 flex items-end justify-center p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="absolute inset-0 bg-gradient-to-t from-orange-500 to-orange-500/0 opacity-60"></div>
                <span className="relative text-white text-xs sm:text-md font-light text-center">{img.description}</span>
              </div>
            </div>
          ))}
        </div>
